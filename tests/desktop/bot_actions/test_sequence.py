...
def test_build_bot_action_sequence_places_slot_1_last() -> None:
    ...
    assert [step.max_scroll_attempts for step in sequence.steps] == [4, 0]
...
def test_build_bot_action_sequence_assigns_scroll_budget_to_first_enabled_slot() -> None:
    result = build_bot_action_sequence(
        (
            BotActionSlotConfig(
                key="slot_1_r",
    ...
    assert [step.max_scroll_attempts for step in result.sequence.steps] == [4, 0]
