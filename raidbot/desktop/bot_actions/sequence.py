...
def _build_slot_step(slot: BotActionSlotConfig) -> AutomationStep:
...
def build_bot_action_sequence(slots: Sequence[BotActionSlotConfig]) -> AutomationSequence:
    return AutomationSequence(
        id="bot-actions",
        name="Bot Actions",
        steps=[_build_slot_step(slot) for slot in slots if slot.enabled and slot.template_path is not None],
    )


def build_slot_test_sequence(slot: BotActionSlotConfig) -> AutomationSequence:
    return AutomationSequence(
        id=f"slot-test-{slot.key}",
        name=f"Test {slot.label}",
        steps=[_build_slot_step(slot)],
    )
