...
class AutomationStorage:
    ...
    def load_sequences(self) -> list[AutomationSequence]:
        ...
        if schema_version is None:
            return self._legacy_sequences_from_data(data)
        return self._current_sequences_from_data(data)
    ...
    def _current_sequences_from_data(self, data: dict[str, Any]) -> list[AutomationSequence]:
        sequences_data = data.get("sequences", [])
        return [self._sequence_from_data(item, schema_version=data.get("schema_version")) for item in sequences_data]

    def _legacy_sequences_from_data(self, data: dict[str, Any]) -> list[AutomationSequence]:
        sequences_data = data.get("sequences", [])
        return [self._legacy_sequence_from_data(item) for item in sequences_data]
    ...
    def _sequence_from_data(self, data: dict[str, Any], schema_version: Any) -> AutomationSequence:
    ...
    def _legacy_sequence_from_data(self, data: dict[str, Any]) -> AutomationSequence:
        return AutomationSequence(
            id=str(data["id"]),
            name=str(data["name"]),
            target_window_rule=self._target_window_rule_from_data(data.get("target_window_rule")),
    ...
    def _legacy_step_from_data(self, data: dict[str, Any]) -> AutomationStep:
        template_path = Path(data["template_path"])
        return AutomationStep(
            name=str(data["name"]),
            template_path=template_path,
